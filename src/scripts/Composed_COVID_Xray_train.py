...
# customized packages
# from src.lib.COVID_Xray_dataset import CovidXrayDataset
from src.lib.helper_func import *

class COVID_Xray_Sys(pl.LightningModule):
    ...
    def train_dataloader(self):
        ...
        # data
        trainset = torchvision.datasets.ImageFolder(root = self.dataset_info["train"],
    ...
    def val_dataloader(self):
        ...
        valset = torchvision.datasets.ImageFolder(root=self.dataset_info["val"],
                                                    transform=val_transformer)
        dataloader =DataLoader(valset, batch_size=self.hparams.batch_size, drop_last=False, shuffle=False, num_workers=4)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--dataset_name', type=str, default='Compose_COVID-Xray_Pneumonia', help='')
    parser.add_argument('--dataset_info', type=str, default='../../config/dataset_info.json', help='path to datainfo .json file')
    ...
    parser.add_argument('--loss_w1', type=float, default=0.25, help='CrossEntropy loss weight for Normal type(Majority)')

    # Debug Info
    parser.add_argument('--log_histogram', type=bool, default=False, help='')


    parser = COVID_Xray_Sys.add_model_specific_args(parser)
