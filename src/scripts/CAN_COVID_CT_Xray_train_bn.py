...
class Mixed_COVID_CT_Xray_Sys(pl.LightningModule):
    ...
    def training_step(self, batch, batch_idx):
        ...
        for i in range(len(label)):
            if ds_label[i] == 0: # ct
                data_ct_lst.append(data[i])
                label_ct_lst.append(label[i])
            else: # xray
                data_xray_lst.append(data[i])
                label_xray_lst.append((label[i]))

        label_xray = torch.empty(0)
        output_xray = torch.empty(0)
        label_ct = torch.empty(0)
        output_ct = torch.empty(0)
        if len(data_xray_lst) > 0:
            data_xray = torch.stack(data_xray_lst)
            label_xray = torch.stack(label_xray_lst)
            # xray
            self.model.set_bn_domain(self.bn_domain_map[self.source_name])
            output_xray = self(data_xray)['logits']

        if len(data_ct_lst) >0:
            data_ct = torch.stack(data_ct_lst)
            label_ct = torch.stack(label_ct_lst)
            # ct
            self.model.set_bn_domain(self.bn_domain_map[self.target_name])
            output_ct = self(data_ct)['logits']


        if len(data_xray_lst) == 0:
            label = label_ct
            output = output_ct
        elif len(data_ct_lst) == 0:
            label = label_xray
            output = output_xray
        else:
            label = torch.cat([label_xray, label_ct], 0)
            output = torch.cat([output_xray, output_ct], 0)

        # DONE: figure out how to concat this
    ...
    def validation_step(self, batch, batch_idx):
        ...
        for i in range(len(label)):
            if ds_label[i] == 0:  # ct
                data_ct_lst.append(data[i])
                label_ct_lst.append(label[i])
                ds_label_0.append(ds_label[i])
            else:  # xray
                data_xray_lst.append(data[i])
                label_xray_lst.append((label[i]))
                ds_label_1.append(ds_label[i])

        label_xray = torch.empty(0)
        output_xray = torch.empty(0)
        ds_label_xray = torch.empty(0)
        label_ct = torch.empty(0)
        output_ct = torch.empty(0)
        ds_label_ct = torch.empty(0)
        if len(data_xray_lst) > 0:
            data_xray = torch.stack(data_xray_lst)
            label_xray = torch.stack(label_xray_lst)
            ds_label_xray = torch.stack(ds_label_1)
            # xray
            self.model.set_bn_domain(self.bn_domain_map[self.source_name])
            output_xray = self(data_xray)['logits']
        ...
        if len(data_ct_lst) > 0:
            data_ct = torch.stack(data_ct_lst)
            label_ct = torch.stack(label_ct_lst)
            ds_label_ct = torch.stack(ds_label_0)
            # ct
            self.model.set_bn_domain(self.bn_domain_map[self.target_name])
            output_ct = self(data_ct)['logits']
        ...
        if len(data_xray_lst) == 0:
            label = label_ct
            output = output_ct
            ds_label = ds_label_ct
        elif len(data_ct_lst) == 0:
            label = label_xray
            output = output_xray
            ds_label = ds_label_xray
        else:
            label = torch.cat([label_xray, label_ct], 0)
            output = torch.cat([output_xray, output_ct], 0)
            ds_label = torch.cat([ds_label_xray, ds_label_ct], 0)

        criterion = nn.CrossEntropyLoss()
        loss = criterion(output, label.long())
        # compute confucsion matrix on this batch
        pred = output.argmax(dim=1).view_as(label)
        return {'val_loss': loss, "pred": pred, "label": label, "ds_label": ds_label}
    ...
    def classAwareLoad(self):
        ...
        # xray dataloader
        self.trainset_xray = CovidXray2clsDataset(xray, train=True, transform=self.train_transformer)
        dataloader_xray = DataLoader(self.trainset_xray, drop_last=False, shuffle=True,
                                     num_workers=4)

        # CT dataloader
        self.trainset_ct = CovidCTDataset(root_dir=ct["image_folder"],
        ...
        dataloader_ct = DataLoader(self.trainset_ct, drop_last=False, shuffle=True,
                                   num_workers=4)
